...
def test_create_project(path_to_images):
    ...
    create_project_all_option_type(api_key, api_secret, client_id, email, path_to_images)
    
    print("\n 2:project with polygon and bounding box")
    create_project_polygon_boundingbox_project(api_key, api_secret, client_id, email, path_to_images)
    
    print("\n 3:project with select, dropdown and radio")
    create_project_select_dropdown_radio(api_key, api_secret, client_id, email, path_to_images)
    ...
    create_project_input_select_radio(api_key, api_secret, client_id, email, path_to_images)
    
    print("\n 6:project with bounding box, dropdown and input")
    create_project_boundingbox_dropdown_input(api_key, api_secret, client_id, email, path_to_images)
...
def test_pre_annotation_uploading(project_id, annotation_format, annotation_file):
    ...
    pre_annotation_uploading(api_key, api_secret, client_id, project_id, annotation_format, annotation_file)
    print("\n Pre-annotation uploading completed.")
    
if __name__ == "__main__":
    
    test_dataset_path = r'D:\professional\LABELLERR\Task\LABIMP-7059-SDK-Testing\test_img_6'
