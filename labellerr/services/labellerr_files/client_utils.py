...
class LabellerrVideoFile(LabellerrFile):
    ...
    def _download_single_frame(self, frame_number, frame_url, save_path, print_lock):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_info = {"frame": frame_number, "status": response.status_code}
                with print_lock:
                    print(
                        f"Failed to download frame {frame_number}: Status {response.status_code}"
                    )

                return False, frame_number, error_info

        except Exception as e:
            error_info = {"frame": frame_number, "error": str(e)}
            with print_lock:
    ...
    def download_frames(
        self, frames_data: dict, output_folder: str | None = None, max_workers: int = 10
    ):
        """
...
if __name__ == "__main__":
    ...
    lb_file = LabellerrFile(
        client=client, file_id=file_id, project_id=project_id, dataset_id=dataset_id
    )
    ...
    lb_dataset = LabellerrVideoDataset(
